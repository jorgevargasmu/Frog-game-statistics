...
# We are going to fit general mixed linear models on the frog game data to figure out which factors affect accuracy and reaction time.
...
## Before we fit our actual data, let's go over what a good model fit looks like:

Visualizing diagnostic plots such as histograms of residuals, Q-Q plots, and residuals vs. fitted value plots are crucial for evaluating the assumptions behind a linear model. Here's what you should look for in each type of plot for it to be considered "good" or indicative of a well-fitting model:
...
p1 <- ggplot(data, aes(x=residuals)) +
  geom_histogram(bins=20, fill="blue", alpha=0.7) +
  labs(title="Histogram of Residuals", x="Residuals", y="Frequency")

# Q-Q plot of Residuals
p2 <- ggplot() +
  stat_qq(aes(sample=residuals), color="blue", alpha=0.5) +
  stat_qq_line(aes(sample=residuals), color="red", linetype="dashed") +
  labs(title="Q-Q Plot of Residuals")

# Residuals vs. Fitted Values
p3 <- ggplot(data, aes(x=fitted(model), y=residuals)) +
...
Histogram of Residuals:
Residuals should form a bell-shaped curve centered around zero, indicating normal distribution.
No significant skew should be present; skewness suggests outliers or unaccounted transformations in the response variable.
Q-Q Plot:
Compares residual quantiles to a standard normal distribution.
Ideal plot: points align on a straight diagonal line (slope = 1, passing through the origin).
Deviations from this line indicate non-normality, such as heavy tails or skewness.
Residuals vs. Fitted Values Plot:
Checks for non-linear patterns and equal variance.
Points should be randomly scattered around the horizontal axis (residual = 0).
No clear patterns should be visible; patterns suggest non-linear relationships or heteroscedasticity.
Residual spread should be consistent across fitted values; varying spread indicates heteroscedasticity.

```{r }
data <- read_csv("../IN/filtered_data.csv")
...
## What is the average reaction time marginalizing over everything?
...
## Which condition is the hardest overall?
